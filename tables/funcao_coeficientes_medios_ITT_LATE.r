...
f_oppen_estima_ITT_LATE     <- function(dados,
                                        ...
                                        var_heterogeneo,         # variável efeitos heterogêneos
                                        output_path
) {
    ...
    for (i in seq_along(vars_resultado)) {
      ...
      for (t in 1:tempo_final) {
        ...
        for(w in seq_along(var_tratamento_recebido)) {
          ...
          for (h in seq_along(c(var_heterogeneo))) {
            ...
            for(c in seq_along(vars_cluster)) {
              ...
              formula_ipw     <- formula(paste("df[[var_tratamento_sorteado]][df$tempo == 0] ~ ", vars_controle))
              ...
              if (!(tipo_metodo %in% c("Lee Bounds - Upper","Lee Bounds - Lower") & length(unique(df[!is.na(df[[var]]), ]$tempo)) < 2)) { # não rodar as regressões do Lee Bounds quando variável não tive linha de base
                ...
                model_ITT     <- lm(df[[var]][tempo == t] ~ df$tratamento_sorteado[tempo == t] + tratamento_dummy_sorteado[df$tempo == 0] + df$hetero_dummy[df$tempo == 0], data = df, weights = weight)
                model_ITT_DID <- lm(df_did[[var]] ~ tratamento_sorteado_x_tempo + tratamento_sorteado + tempo + tratamento_dummy_sorteado + hetero_dummy, data = df_did, weights = weight_did)
                
                formula_ITT_control <- formula(paste0(var, "[tempo == ", t, "] ~ tratamento_sorteado[tempo == ", t, "] + tratamento_dummy_sorteado[df$tempo == 0] + df$hetero_dummy[df$tempo == 0] + ", vars_controle))
                ...
                model_LATE     <- ivreg(df[[var]][tempo == t] ~ tratamento_recebido[tempo == t] + tratamento_dummy_recebido[df$tempo == 0] + hetero_dummy[df$tempo == 0] | tratamento_sorteado[tempo == t] + tratamento_dummy_sorteado[df$tempo == 0] + hetero_dummy[df$tempo == 0], data = df, weights = weight) # Para o teste Weak-instrument um p-valor baixo indica que há forte evidência contra a hipótese nula de que os instrumentos são fracos. Isso sugere que os instrumentos são relevantes para a variável instrumental, o que é desejável. O teste de Wu-Hausman é usado para testar a consistência dos estimadores IV em relação aos estimadores OLS. Um p-valor alto indica que não há evidências significativas para rejeitar a hipótese nula de consistência entre os estimadores IV e OLS. Isso sugere que o modelo IV pode ser consistente com o modelo OLS.
                model_LATE_DID <- ivreg(df_did[[var]] ~ tratamento_recebido_x_tempo + tratamento_recebido + tempo + tratamento_dummy_recebido + hetero_dummy | tratamento_sorteado_x_tempo + tratamento_sorteado + tempo + tratamento_dummy_sorteado + hetero_dummy, data = df_did, weights = weight_did)
                
                formula_LATE_control <- formula(paste0(var, "[tempo == ", t, "] ~ tratamento_recebido[tempo == ", t, "] + tratamento_dummy_recebido[df$tempo == 0] + hetero_dummy[df$tempo == 0] + ", vars_controle, " | ", "tratamento_sorteado[tempo == ", t, "] + tratamento_dummy_sorteado[df$tempo == 0] + hetero_dummy[df$tempo == 0] + ", vars_controle))
