                      ...
                      if (tipo_regressao == "Diferença de Médias") {
                        if (tipo_estimador == "ITT") {
                          if(tipo_controle == "não"){
                            model <- lm(df[[var]][tempo == t] ~ df$tratamento_sorteado[tempo == t] + tratamento_dummy_sorteado[df$tempo == baseline] + df$hetero_dummy[df$tempo == baseline], data = df, weights = weight)
                          }
                          if(tipo_controle == "sim"){
                            formula <- formula(paste0(var, "[tempo == ", t, "] ~ tratamento_sorteado[tempo == ", t, "] + tratamento_dummy_sorteado[tempo == baseline] + hetero_dummy[tempo == baseline] + ", vars_controle))
                        ...
                        if (tipo_estimador == "LATE" & t != 0){
                          if (var_trat_receb != var) {
                            ...
                            if(tipo_controle == "sim"){
                              formula <- formula(paste0(var, "[tempo == ", t, "] ~ tratamento_recebido[tempo == ", t, "] + tratamento_dummy_recebido[df$tempo == baseline] + hetero_dummy[df$tempo == baseline] + ", vars_controle, " | ", "tratamento_sorteado[tempo == ", t, "] + tratamento_dummy_sorteado[df$tempo == baseline] + hetero_dummy[df$tempo == baseline] + ", vars_controle))
